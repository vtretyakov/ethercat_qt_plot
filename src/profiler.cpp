...
Profiler::Profiler()
{
    motion_profile = new MotionProfile();
}

Profiler::~Profiler()
{
    delete motion_profile;
}

void Profiler::init_position_profile_limits(MotionProfile *motion_profile, int max_torque, int max_torque_acceleration, int max_acceleration, int max_velocity, int max_position, int min_position, int ticks_per_turn)
{
...
int Profiler::init_position_profile(MotionProfile *motion_profile, int target_position, int actual_position, int velocity, int acceleration, int deceleration, int ticks_per_turn) {
...
int Profiler::position_profile_generate(MotionProfile *motion_profile, int step) {
...
int Profiler::init_velocity_profile(MotionProfile *motion_profile, int target_velocity, int actual_velocity, int acceleration, int deceleration, int ticks_per_turn)
{
...
int Profiler::linear_profile_generate_in_steps(MotionProfile *motion_profile, int step)
{
    return (int) round( motion_profile->qid + motion_profile->acc * motion_profile->s_time * step);
}

int Profiler::init_torque_profile(MotionProfile *motion_profile, int target_torque, int actual_torque, int acceleration, int deceleration)
{
...
int Profiler::init_linear_profile(MotionProfile *motion_profile)
{
...
float Profiler::ticks_to_rpm(float ticks, int ticks_per_turn) {
    return (ticks*(float)60)/(float)ticks_per_turn;
}

float Profiler::rpm_to_ticks(int rpm, int ticks_per_turn) {
